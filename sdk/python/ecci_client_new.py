...
class MqttClient:
    ...
    def _on_client_connect(self, mqtt_client, userdata, flags, rc):
        ...
        # 应用为开始，连接后首先发送ready 消息
        if not self.training_start.isSet():
            self.send_ready()
        ...
        if rev_msg['type'] == "cmd":
            ...
            try:
                if rev_msg['contents']['cmd'] == "start":
                    logger.debug(f"{CONTAINER_NAME} received the start command from {sender}")
        ...
        if sender in LISTEN_CONTAINERS:
            ...
            if all(list(LISTEN_CONTAINERS.values())):
                ...
                if ECCI_APP_CONTROLLER_CONTAINER:
                    ...
                    payload = {"type":"cmd","contents":{"cmd":"start"}}
                    self.publish(message=payload)
                    logger.debug("send start message")
                    try:
                        print("self.training_start.set()="+str(self.training_start.isSet()))
                    except Exception as e:
                        print(e)
    ...
    def send_ready(self):
        ...
        if ECCI_APP_TYPE == "controller":
            # TODO(gyf)：发送ready给brokermonitor，需配置协商
            logger.debug("controller send ready")
            # topic=f"toEdgeAI/{broker_id}/{APP_ID}/app/{CONTAINER_NAME}/{container_name}"
            payload = {"app_id":APP_ID,"container_name":CONTAINER_NAME,"agent_id":ECCI_AGENT_ID,"container_type":ECCI_APP_TYPE}
...
if __name__ == "__main__":
    ...
    mqtt_thread.start()
    print("test")
    mqtt_client.training_start.wait()
    print("start")
